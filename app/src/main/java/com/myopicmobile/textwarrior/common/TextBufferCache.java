...
public class TextBufferCache {
	...
	private Pair<Integer>[] _cache = new Pair[CACHE_SIZE];
	
	public TextBufferCache(){
		_cache[0] = new Pair<Integer>(0, 0); // invariant lineIndex and charOffset relation
		for (int i = 1; i < CACHE_SIZE; ++i){
			_cache[i] = new Pair<Integer>(-1, -1);
	...
	public Pair<Integer> getNearestLine(int lineIndex){
		int nearestMatch = 0;
		...
		Pair<Integer> nearestEntry = _cache[nearestMatch];
		makeHead(nearestMatch);
		return nearestEntry;
	}
	
	public Pair<Integer> getNearestCharOffset(int charOffset){
		int nearestMatch = 0;
		int nearestDistance = Integer.MAX_VALUE;
		for(int i = 0; i < CACHE_SIZE; ++i){
	...
	private void makeHead(int newHead){
		...
		Pair<Integer> temp = _cache[newHead];
